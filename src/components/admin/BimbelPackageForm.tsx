...
interface BimbelPackageFormProps {
  packageId: string;
}

export default function BimbelPackageForm({ packageId }: BimbelPackageFormProps) {
  const [nama, setNama] = useState('');
  const [profesi, setProfesi] = useState<'bidan' | 'perawat'>('bidan');
  const [jenjang, setJenjang] = useState<'D3' | 'D4-S1'>('D3');
  const [durasi_bulan, setDurasiBulan] = useState(0);
  ...
  useEffect(() => {
    if (!isNew) {
      setLoading(true);
      bimbelPackageService.getBimbelPackageById(packageId)
        .then((data: BimbelPackage) => {
          setNama(data.nama);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isNew) {
        await bimbelPackageService.createBimbelPackage(formData);
      } else {
        await bimbelPackageService.updateBimbelPackage(packageId, formData);
      }
      router.push('/admin/bimbel-packages');
    } catch (err: any) {
