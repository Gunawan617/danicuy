...
import { bimbelPackageService } from '@/lib/api';
import { BimbelPackage } from '@/types';

export default function BimbelPackagesPage() {
  const [packages, setPackages] = useState<BimbelPackage[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    bimbelPackageService.getAllBimbelPackages()
      .then(data => {
        setPackages(data);
        setLoading(false);
      })
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Manage Bimbel Packages</h1>
        <Link href="/admin/bimbel-packages/new" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Add New Package
        </Link>
      </div>
            ...
            {packages.map(pkg => (
              <tr key={pkg._id}>
                ...
                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                  <Link href={`/admin/bimbel-packages/${pkg._id}`} className="text-indigo-600 hover:text-indigo-900 mr-4">Edit</Link>
